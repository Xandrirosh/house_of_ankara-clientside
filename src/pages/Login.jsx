    ...
    <>
      <section className='w-full container mx-auto px-2'>
        <div className='bg-white my-4 w-full max-w-lg mx-auto rounded p-7'>
          <p className='flex justify-center text-black font-bold text-2xl'>Login To Account</p>
          <form className='grid gap-4 mt-6 border p-2 rounded' onSubmit={handleSubmit}>
            ...
            <div className='grid gap-1'>
              ...
              <Link to={'/forgot-password'} className='block ml-auto hover:text-blue-800 text-sm'>Forgot password ?</Link>
            </div>

            <button disabled={!validateValue} className={`${validateValue ? 'bg-gray-600 hover:bg-black' : 'bg-gray-500'}
             text-white font-semibold rounded py-2 tracking-wide`}>
              Login
            </button>
          </form>
          <p className='text-center text-sm'>
            Don't have an account ? <Link to={'/register'}
              className='font-semibold text-blue-700 hover:text-blue-800'>Register</Link>
          </p>
        </div>
      </section>
    </>
